...
type Data = FinalInspectionData['insp_time_po_info'];
    ...
    <View>
      <SectionCard title="Inspection Time & PO Info" subtitle="Basic inspection details">
        <FormRow>
          ...
          <TextField label="Type" labelCn="类型" value={data.type || 'Final'} onChangeText={(v) => onChange({ type: v })} />
          ...
          <MultiSelectDropdown label="Reporting User" values={data.reporting_user} options={[{ label: 'User 1', value: 'User 1' }, { label: 'User 2', value: 'User 2' }, { label: 'User 3', value: 'User 3' }]} onChangeValues={(v) => onChange({ reporting_user: v })} />
          <MultiSelectDropdown label="Second Reporting User" values={data.second_reporting} options={[{ label: 'User 1', value: 'User 1' }, { label: 'User 2', value: 'User 2' }, { label: 'User 3', value: 'User 3' }]} onChangeValues={(v) => onChange({ second_reporting: v })} />
          <TextField label="Inspector" labelCn="检验员" value={data.inspector || useAuthStore.getState().user?.name || ''} editable={false} onChangeText={() => {}} />
      ...
      <SectionCard title="PO Selection" subtitle="Select POs and Styles">
        {data.styles.length === 0 && (
          <Text className="text-body text-textSecondary italic py-4">No POs added.</Text>
        )}
