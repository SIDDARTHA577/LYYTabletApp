...
export function FinalInspectionListScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await listInspections('final_inspection');
      setItems(res.items);
      setDemoMode(false);
    } catch {
      // API unreachable — fall back to static demo dataset
      setItems(mockFinalInspections as unknown as InspectionSummary[]);
      setDemoMode(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppHeader title="Final Inspection" subtitle="End of production review" showBack />
      {demoMode && (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <InspectionListItem
            item={item}
            onPress={() => navigation.navigate('FinalInspectionForm', { inspectionId: item._id.startsWith('fi-mock-') ? undefined : item._id })}
          />
        )}
        ListEmptyComponent={
          !loading ? (
            items.length === 0 ? (
              <EmptyState
                ...
                description="Start a new Final Inspection."
                actionLabel={t('newInspection')}
                onAction={() => navigation.navigate('FinalInspectionForm')}
              />
            ) : (
      ...
      <FAB
        ...
        onPress={() => navigation.navigate('FinalInspectionForm')}
      />
