...
export function DailyCheckListScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await listInspections('daily_check');
      setItems(res.items);
      setDemoMode(false);
    } catch {
      // API unreachable — fall back to static demo dataset
      setItems(mockDailyChecks as unknown as InspectionSummary[]);
      setDemoMode(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppHeader title="Daily Check" subtitle="Production Follow-Up Report" showBack />
      {demoMode && (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <InspectionListItem
            item={item}
            onPress={() => navigation.navigate('DailyCheckForm', { inspectionId: item._id.startsWith('dc-mock-') ? undefined : item._id })}
          />
        )}
        ListEmptyComponent={
          !loading ? (
            items.length === 0 ? (
              <EmptyState
                ...
                title="No Daily Checks yet"
                description="Start a new Daily Check Production Follow-Up Report."
                actionLabel={t('newInspection')}
                onAction={() => navigation.navigate('DailyCheckForm')}
              />
            ) : (
      ...
      <FAB
        ...
        onPress={() => navigation.navigate('DailyCheckForm')}
      />
