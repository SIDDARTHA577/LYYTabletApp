...
export function PpMeetingListScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await listInspections('pp_meeting');
      setItems(res.items);
      setDemoMode(false);
    } catch {
      // API unreachable — fall back to static demo dataset
      setItems(mockPpMeetings as unknown as InspectionSummary[]);
      setDemoMode(true);
    } finally {
      setLoading(false);
    }
  ...
  const visible = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((i) => {
        ...
        return [info.vendor, info.style, info.po].filter(Boolean).some((v: string) => v.toLowerCase().includes(q));
      });
    }
    return list;
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppHeader title="PP Meeting" subtitle="Pre-Production Meeting records" showBack />
      {demoMode && (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <InspectionListItem
            item={item}
            onPress={() => navigation.navigate('PpMeetingForm', { inspectionId: item._id.startsWith('ppm-mock-') ? undefined : item._id })}
          />
        )}
        ListEmptyComponent={
          !loading ? (
            items.length === 0 ? (
              <EmptyState
                icon="progress-clock"
                title="No PP Meetings yet"
                description="Start a new PP Meeting record."
                actionLabel={t('newInspection')}
                onAction={() => navigation.navigate('PpMeetingForm')}
              />
            ) : (
      ...
      <FAB
        ...
        onPress={() => navigation.navigate('PpMeetingForm')}
      />
