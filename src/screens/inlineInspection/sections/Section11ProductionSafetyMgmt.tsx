...
type Data = InlineInspectionData['production_safety_mgmt'];
...
export function Section11ProductionSafetyMgmt({ data, onChange }: { data: Data; onChange: (next: Partial<Data>) => void }) {
      ...
      <SectionCard title="Production Safety Management Checklist" subtitle="Safety checks with supporting documents">
        <List.AccordionGroup>
              ...
              <View className="mb-4">
                <DropdownField
                  ...
                  options={[
                    ...
                    { label: 'Sweater factory Knitting Machine Needles', value: 'Sweater' },
          ...
          <List.Accordion id="2" title="2. Sharp Object Management" titleStyle={{ fontWeight: 'bold' }}>
            <View className="p-4 bg-surface">
              <SharpObjectGroup label="Knives" dataKey="knives" data={sharp} onChange={(v) => onChange({ sharp_object_management: v })} />
              <SharpObjectGroup label="Blades" dataKey="blades" data={sharp} onChange={(v) => onChange({ sharp_object_management: v })} />
              <SharpObjectGroup label="Snippers" dataKey="snippers" data={sharp} onChange={(v) => onChange({ sharp_object_management: v })} />
              <SharpObjectGroup label="Box Cutters" dataKey="box_cutters" data={sharp} onChange={(v) => onChange({ sharp_object_management: v })} />
              <SharpObjectGroup label="Plastic Pick" dataKey="plastic_pick" data={sharp} onChange={(v) => onChange({ sharp_object_management: v })} />
            </View>
          </List.Accordion>
