...
type Data = InlineInspectionData['accessories'];



export function Section5Accessories({ data, onChange }: { data: Data; onChange: (next: Partial<Data>) => void }) {
  ...
  return (
    <View>
      <SectionCard title="Accessories" subtitle="Select all accessories present / applicable">
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 16 }}>
          {ACCESSORIES.map((item) => (
            <View key={item.key} className="flex-row items-center min-w-[200px]">
              <Checkbox.Android
                status={data[item.key] ? 'checked' : 'unchecked'}
                onPress={() => onChange({ [item.key]: !data[item.key] })}
              />
