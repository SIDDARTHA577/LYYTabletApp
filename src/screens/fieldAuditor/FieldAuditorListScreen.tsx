...
export function FieldAuditorListScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await listInspections('field_auditor');
      setItems(res.items);
      setDemoMode(false);
    } catch {
      // API unreachable — fall back to static demo dataset
      setItems(mockFieldAuditorInspections as unknown as InspectionSummary[]);
      setDemoMode(true);
    } finally {
      setLoading(false);
    }
  ...
  const visible = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((i) => {
        const info = (i.data as any)?.cover_summary ?? {};
        // Match on factory
        return [info.factory_name_en, info.factory_name_cn].filter(Boolean).some((v: string) => v.toLowerCase().includes(q));
      });
    }
    return list;
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppHeader title="Factory Check" subtitle="Field Auditor Report" showBack />
      {demoMode && (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <InspectionListItem
            item={item}
            onPress={() => navigation.navigate('FieldAuditorForm', { inspectionId: item._id.startsWith('fa-mock-') ? undefined : item._id })}
          />
        )}
        ListEmptyComponent={
          !loading ? (
            items.length === 0 ? (
              <EmptyState
                icon="domain"
                title="No Factory Checks yet"
                description="Start a new Factory Check (Field Auditor)."
                actionLabel={t('newInspection')}
                onAction={() => navigation.navigate('FieldAuditorForm')}
              />
            ) : (
      ...
      <FAB
        ...
        onPress={() => navigation.navigate('FieldAuditorForm')}
      />
